...
# delay_range = Range(10813000,10815000, 1)
delay_range = Range(11322500,11342500, 1)
width_range = Range(100, 2800, 100)

# width_range = Range(1500, 2800, 100)

# width_range = Range(150, 280, 100)
repeat_range = Range(1,1, 1)
    ...
    while delay <= delay_range.max:
        ...
        while width <= width_range.max:
            while repeat <= repeat_range.max:
                ...
                exit_status = os.system('openocd -s /opt/homebrew/Cellar/open-ocd/0.11.0/share/openocd/scripts -f ./interface/jlink.cfg -c "transport select swd" -f ./target/nrf52.cfg -c "init;dump_image nrf52_flash.bin 0x0 0x80000;exit"')
